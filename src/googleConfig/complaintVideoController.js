...
const stream = require("stream"); // ✅ for handling buffer as stream
...
const updateComplaintWithVideo = async (req, res) => {
  try {
    ...
    let partPendingVideo = complaint.partPendingVideo;
    ...
    if (req.file?.buffer) {
      ...
      const uploadResponse = await drive.files.create({
        requestBody: {
          ...
          parents: [process.env.GOOGLE_DRIVE_FOLDER_ID], // ✅ use ENV
        },
        media: {
          mimeType: req.file.mimetype || "video/mp4",
          body: bufferStream,
      ...
      // Delete old video from Google Drive
      if (complaint.partPendingVideo?.includes("id=")) {
        const oldFileId = complaint.partPendingVideo.split("id=")[1].split("&")[0];
        if (oldFileId && oldFileId !== fileId) {
          await drive.files.delete({ fileId: oldFileId }).catch(console.error);
